...
model.add(tf.layers.dense({ inputShape: [9], units: 1, activation: 'sigmoid' })) // Input 4x + 9 hidden
//model.add(tf.layers.dense({ units: 9, activation: 'sigmoid' })) // 5 hidden
model.add(tf.layers.dense({ units: 1, activation: 'sigmoid' })) // 1 output
...
function processData(data) {
    ...
    for (let line of lines) {
        ...
        values.shift() // take one from beginning
        for (let i = 0; i < values.length; i++) {
            if (!isNaN(values[i])) {
                values[i] = Number.parseFloat(values[i]) / 10; // change 'numbers' to actual numbers
            }
        }
        if (Math.random() < testRatio) {
            test.x.push(values.splice(0, 9)); // place first 9 values into x
            test.y.push(values[0] == 0.2 ? [0] : [1]); // place rest 3 into y 
        } else {
            train.x.push(values.splice(0, 9));
            train.y.push(values[0] == 0.2 ? [0] : [1]);
        }
...
loadData('data/breast-cancer-wisconsin.data');
